...
//截取小数点后八位 如果没有小数点 自动补0
+ (NSString *)interceptTheEightBitsAfterDecimalPoint:(NSString *)string{
    ...
    if ([string containsString:@"."]) {
        ...
        if (bits.count == 2) {
            ...
            NSString *newBit = @"";
            if (bit.length >= 8) {
                newBit = [bit substringToIndex:8];
            }else{
                newBit = bit;
                for (int i = 0; i < 8 - bit.length; i++) {
                    newBit = [newBit stringByAppendingFormat:@"0"];
                }
            }
