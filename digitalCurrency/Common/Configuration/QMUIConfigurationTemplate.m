...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarButtonFont = UIFontMake(18);                                  // NavBarButtonFont : QMUINavigationButtonTypeNormal 的字体（由于系统存在一些 bug，这个属性默认不对 UIBarButtonItem 生效）
...
//    QMUICMI.navBarTintColor = UIColorWhite;                                     // NavBarTintColor : QMUINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
    QMUICMI.navBarTitleColor = UIColorMake(36, 58, 46);                                 // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
...
//    QMUICMI.navBarBackIndicatorImage = [[UIImage qmui_imageWithShape:QMUIImageShapeNavBack size:CGSizeMake(12, 20) tintColor:NavBarTintColor]];
//    QMUICMI.navBarBackIndicatorImage = [UIImage imageNamed:@"返回白"];
...
- (UIColor *)themeTitleTextColor {
    return UIColorMake(31, 63, 89);
}

- (UIColor *)themeMainTextColor {
    return UIColorMake(140, 159, 173);
}

- (UIColor *)themeDescriptionTextColor {
    return UIColorMake(208, 216, 228);
}
...
- (UIColor *)themeBorderColor{
    return UIColorMake(204, 204, 204);
}

- (UIColor *)themePlaceholderColor {
    return UIColorMake(207, 212, 216);
}
...
- (UIColor *)themeSeparatorColor {
    return UIColorMake(238, 238, 238);
}
