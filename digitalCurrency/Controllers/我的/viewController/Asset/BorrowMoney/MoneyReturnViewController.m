...
- (void)layoutUI{
    ...
    self.boAmountLabel.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.rateLabel.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.boAmountValueLabel.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.rateValueLabel.textColor = QDThemeManager.currentTheme.themeMainTextColor;

    self.reAmountLabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.haveReturnLabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
...
- (void)initData{
    ...
    self.haveReturnLabel.text = [NSString stringWithFormat:@"%@:%@ %@", LocalizationKey(@"shouldbereturned"), [ToolUtil reviseString:[[can decimalNumberByAdding:loan] stringValue]], self.model.coin.unit];
    
    self.boAmountLabel.text = LocalizationKey(@"Quantityofloans");
    self.boAmountValueLabel.text = [NSString stringWithFormat:@"%@ %@",[ToolUtil reviseString:self.model.loanBalance], self.model.coin.unit];
    
    self.rateLabel.text = LocalizationKey(@"Interest");
    self.rateValueLabel.text = [NSString stringWithFormat:@"%@ %@",[ToolUtil reviseString:self.model.accumulative], self.model.coin.unit];
...
- (IBAction)allAction:(UIButton *)sender {
    ...
    NSDecimalNumber *can = [[NSDecimalNumber alloc] initWithString:self.model.amount];
    NSDecimalNumber *loan = [[NSDecimalNumber alloc] initWithString:self.model.accumulative];
    self.amountTF.text = [ToolUtil reviseString:[[can decimalNumberByAdding:loan] stringValue]];
}
