...
-(UIView *)backview{
    if (!_backview) {
        _backview = [[UIView alloc]initWithFrame:CGRectMake(0, 100, kWindowW, 15)];
        _backview.backgroundColor = [UIColor clearColor];
        NSInteger num = self.dataSourceArr.count % 3 > 0 ? self.dataSourceArr.count / 3 + 1 : self.dataSourceArr.count / 3;
        for (int i = 0; i < num; i ++) {
            UILabel *linelabel = [[UILabel alloc]initWithFrame:CGRectMake(((kWindowW - (15 * num + 2 *(num - 1))) / 2 + 15 * i + 2 * (i - 1)), 5, 15, 2)];
            linelabel.tag = i;
            if (i == 0) {
                linelabel.backgroundColor = [QDThemeManager currentTheme].themeTitleTextColor;
            }else{
                linelabel.backgroundColor = [QDThemeManager currentTheme].themeSeparatorColor;;
            }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake(kWindowW / 3,100);
}
...
- (void)setDataSourceArr:(NSArray *)dataSourceArr{
    ...
    self.collectionView.backgroundColor = [QDThemeManager currentTheme].themeBackgroundColor;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    for (UILabel *label in array) {
        if (label.tag == num) {
            label.backgroundColor = [QDThemeManager currentTheme].themeTitleTextColor;
        }else{
            label.backgroundColor = [QDThemeManager currentTheme].themeSeparatorColor;
