...
@interface RegisterViewController ()<QMUITextFieldDelegate>
{
     BOOL _isEmail;//邮箱注册
    NSDictionary *_g3Result;
}
@property (weak, nonatomic) IBOutlet QMUITextField *phoneNumberTF;
@property (weak, nonatomic) IBOutlet QMUITextField *verifyCodeTF;
...
@property (weak, nonatomic) IBOutlet QMUITextField *ConfirmpasswordTF;//确认密码
...
@property (weak, nonatomic) IBOutlet QMUITextField *invitaTF;
...
@property (weak, nonatomic) IBOutlet QMUITextField *countryTF;
...
- (void)layoutUI{
    ...
    self.countryLabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.usernamelabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.phonelabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.verifycodelabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.passwordlabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.confirmlabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;
    self.invitaLabel.textColor = QDThemeManager.currentTheme.themeTitleTextColor;

    
    self.countryTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.userNameTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.phoneNumberTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.verifyCodeTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.passwordTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.ConfirmpasswordTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    self.invitaTF.textColor = QDThemeManager.currentTheme.themeMainTextColor;
    
    self.countryTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.userNameTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.phoneNumberTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.verifyCodeTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.passwordTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.ConfirmpasswordTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    self.invitaTF.placeholderColor = QDThemeManager.currentTheme.themePlaceholderColor;
    ...
    self.line5.backgroundColor = QDThemeManager.currentTheme.themeSeparatorColor;
    self.line6.backgroundColor = QDThemeManager.currentTheme.themeSeparatorColor;
    self.lineView.backgroundColor = QDThemeManager.currentTheme.themeSeparatorColor;

    [self.agressButton setTitleColor:[QDThemeManager currentTheme].themeDescriptionTextColor forState:UIControlStateNormal];
    self.haveAccountnumber.textColor = QDThemeManager.currentTheme.themeMainTextColor;
...
-(void)getphonncode{
    ...
    if (!_isEmail) {
        NSString *url = [NSString stringWithFormat:@"%@%@",HOST, @"uc/mobile/code"];
        NSDictionary *param = @{@"phone":self.phoneNumberTF.text,
                                @"country":self.countryModel.zhName
                                };
...
//    [codeDic setValue:_g3Result[@"randstr"] forKey:@"randStr"];
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",HOST, @"uc/register/phone"];
    NSDictionary *param = @{@"phone":self.phoneNumberTF.text,
                            @"password":self.passwordTF.text,
                            @"code":self.verifyCodeTF.text,
                            @"username":self.userNameTF.text,
                            @"country":self.countryModel.zhName,
                            @"promotion":self.invitaTF.text,
                            @"ticket":@"",
                            @"randStr":@"",
                            };
    [[XBRequest sharedInstance] postDataWithUrl:url Parameter:param contentType:@"application/x-www-form-urlencoded" ResponseObject:^(NSDictionary *responseResult) {
        [EasyShowLodingView hidenLoding];
        NSLog(@"手机号注册 ---- %@",responseResult);
        if ([responseResult objectForKey:@"resError"]) {
            NSError *error = responseResult[@"resError"];
            [self.view makeToast:error.localizedDescription];
        }else{
            if ([responseResult[@"code"] integerValue] == 0) {
                [self.view makeToast:responseResult[MESSAGE] duration:1.0 position:CSToastPositionCenter];
...
//    [LoginNetManager getAccountForPhone:self.phoneNumberTF.text withpassword:self.passwordTF.text withcode:self.verifyCodeTF.text withusername:self.userNameTF.text withcountry:self.countryModel.zhName withpromotion:self.invitaTF.text withG3Result:@{} CompleteHandle:^(id resPonseObj, int code) {
//
//        [EasyShowLodingView hidenLoding];
//        if (code) {
//            if ([resPonseObj[@"code"] integerValue] == 0) {
