...
- (void)addEntrustSubViewsWithSymbols:(NSArray *)symbols{
    ...
    containerView.backgroundColor = QDThemeManager.currentTheme.themeBackgroundColor;
...
- (HJDropDownMenu *)creatMenuWithTitles:(NSArray *)titles{
    ...
    yearMenu.backgroundColor = QDThemeManager.currentTheme.themeBackgroundColor;
    yearMenu.rowHeight = 35;
    yearMenu.datas = titles;
    yearMenu.textColor = QDThemeManager.currentTheme.themeMainTextColor;;
    return yearMenu;
}
...
//    headerBtn.layer.borderWidth = 0.5;
    ...
    [headerBtn setBackgroundColor:QDThemeManager.currentTheme.themeBackgroundDescriptionColor];
    headerBtn.titleLabel.font = [UIFont systemFontOfSize:12];
    [headerBtn setTitleColor:QDThemeManager.currentTheme.themeMainTextColor forState:UIControlStateNormal];
...
//    datePickManager.view.backgroundColor = [UIColor whiteColor];
    ...
    datePicker.backgroundColor = QDThemeManager.currentTheme.themeBackgroundColor;
    [[AppDelegate sharedAppDelegate] presentViewController:datePickManager animated:YES completion:^{
        
    }];

    datePickManager.headerViewBackgroundColor = QDThemeManager.currentTheme.themeBackgroundDescriptionColor;
    datePickManager.titleLabel.text = LocalizationKey(@"startTime");
    //设置确定按钮的字体颜色
    datePickManager.confirmButtonTextColor = QDThemeManager.currentTheme.themeTitleTextColor;
    //设置选中行的字体颜色
    datePicker.textColorOfSelectedRow = QDThemeManager.currentTheme.themeTitleTextColor;
    //设置线条的颜色
    datePicker.lineBackgroundColor = QDThemeManager.currentTheme.themeSeparatorColor;
    //设置确定按钮的字
